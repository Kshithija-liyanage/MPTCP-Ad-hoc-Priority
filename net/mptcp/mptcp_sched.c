			    ...
			    bool zero_wnd_test, bool *force)
{
	...
	u32 adhoc_srtt = 0xffffffff;
	int adhoc_priority = mpcb->adhoc_priority;
	int lenear_retransmits = 0;
	bool found_unused = false;
	bool found_unused_una = false;
	struct mptcp_tcp_sock *mptcp;
	int interface_type = 1;

	if(mpcb->is_adhoc_avail == 1 && adhoc_priority == 0 && mpcb->adhoc_tries == 1){
		...
		mptcp_debug("%s kshithija %u %u %u \n", __func__, min_srtt, adhoc_srtt, (adhoc_srtt - min_srtt));
		if(bestsk && adhoc_srtt > min_srtt && ((adhoc_srtt - min_srtt) > min_srtt)){
			mpcb->adhoc_priority = 1;
		}
		...
		mptcp_for_each_sub(mpcb, mptcp) {
			...
			if(interface_type == 0){
				adhoc_srtt = tp->srtt_us;
				lenear_retransmits = icsk->lenear_retransmits;
				icsk->lenear_retransmits = 0;
			}
		...
		mptcp_debug("%s kshithija normal return %u %u %u \n", __func__, min_srtt, adhoc_srtt, (adhoc_srtt - min_srtt));
		if(adhoc_srtt != 0xffffffff && ((adhoc_srtt - min_srtt) < min_srtt)){
			mpcb->adhoc_priority = 0;
		}		
...
static struct sk_buff *mptcp_rcv_buf_optimization(struct sock *sk, int penal)
{
	...
	/* Half the cwnd of the slow flows kshithija*/
