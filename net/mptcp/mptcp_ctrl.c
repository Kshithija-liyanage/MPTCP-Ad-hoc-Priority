...
static int proc_mptcp_adhoc(struct ctl_table *ctl, int write,
				void __user *buffer, size_t *lenp,
				loff_t *ppos)
{
	char val[MPTCP_SCHED_NAME_MAX];
...
static struct ctl_table mptcp_table[] = {
	...
	{
		.procname	= "adhoc_interface",
		.mode		= 0644,
		.maxlen		= MPTCP_SCHED_NAME_MAX,
		.proc_handler	= proc_mptcp_adhoc,
	},
	{ }
};
		   ...
		   gfp_t flags)
{
	...
	mpcb->is_adhoc_avail = adhoc_avail;
	mpcb->adhoc_tries = 0;
